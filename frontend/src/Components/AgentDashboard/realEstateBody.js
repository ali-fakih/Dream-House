...
function RealEstate() {
  ...
  const fetchHouses = useCallback(async () => {
    try {
      const response = await axios.get(
        `http://localhost:3000/realestate/getBYagent/${agentId}`
      );
  ...
  const addSellHouse = async (e) => {
    ...
    try {
      ...
      formData.append("userAgent", agentId);
              ...
              <div className="grid grid-cols-3 gap-4 mt-10">
                {realestate.filter(applyFilters).map((realestate, index) => (
                  ...
                  >
                    ...
                    <Link
                      to={`/dashboardAgent/RealEstatePropertyAgent/${realestate._id}`}
